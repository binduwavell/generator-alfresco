...
describe('generator-alfresco:surf-extensions-2-1-1', function () {
  this.timeout(60000);
  ...
  it('create surf extension in existing project', function () {
    return helpers.run(path.join(__dirname, '../generators/surf-extension'))
      ...
      .toPromise()
      .then(dir => {
        const surfExtensionFile = path.join(osTempDir, 'share-amp/src/main/amp/config/alfresco/web-extension/site-data/extensions/test-extension.xml');
        assert.file([
          surfExtensionFile,
        ]);
        assert.fileContent(
  ...
  it('create with invalid version', function () {
    return helpers.run(path.join(__dirname, '../generators/surf-extension'))
      ...
      .then(dir => {
        assert.noFile([
          'share-amp/src/main/amp/config/alfresco/web-extension/site-data/extensions/invalidVersion-extension.xml',
  ...
  it('create with invalid auto deploy', function () {
    return helpers.run(path.join(__dirname, '../generators/surf-extension'))
      ...
      .then(dir => {
        assert.noFile([
          'share-amp/src/main/amp/config/alfresco/web-extension/site-data/extensions/invalidAutoDeploy-extension.xml',
        ].map(relativePath => path.join(osTempDir, relativePath)));
      });
  });
});
